...
program main
  ...
  call output_xyz(0, natom, part_type, coord, velocity, box_length)
  ...
  do nstep = 1, nstep_total
    ...
    ! 3, cal force
    call calc_force(nstep, natom, part_type, coord, vel_dpd, for_dpd, box_length, ene%pot(2))
    ...
    if( mod( nstep-1, dump_ene_step ) .eq. 0 )then
      ...
      call output_energy_dat(dt, nstep, natom, ene)
      ene%potave(1:3) = 0.0d0
      ene%kinave      = 0.0d0
    endif

    if( mod(nstep-1, dump_step).eq. 0)then
      call output_xyz(nstep, natom, part_type, coord, velocity, box_length)
