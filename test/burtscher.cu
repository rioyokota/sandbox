...
{
  register int i, j, k, ch, inc, cnt, bottom, restart, flag;
...
int main(int argc, char *argv[])
{
  ...
  fprintf(stderr, "CUDA BarnesHut v2.2n\n");
  if (argc != 4) {
    fprintf(stderr, "\n");
  ...
  for (run = 0; run < 3; run++) {
...
#ifdef dump
      f = fopen("BHdata.bin", "w+b");
      fwrite(&nbodies, sizeof(int), 1, f);
      fwrite(&timesteps, sizeof(int), 1, f);
#endif
    }
    ...
    for (step = 0; step < timesteps; step++) {
      ...
      cudaEventRecord(start, 0);
      ClearKernel1<<<blocks * 1, 1024>>>();
      TreeBuildingKernel<<<blocks * FACTOR2, THREADS2>>>();
      ClearKernel2<<<blocks * 1, 1024>>>();
      cudaEventRecord(stop, 0);  cudaEventSynchronize(stop);  cudaEventElapsedTime(&time, start, stop);
      timing[2] += time;
...
#ifdef dump
      fwrite(posx, sizeof(float), nbodies, f);
      fwrite(posy, sizeof(float), nbodies, f);
      fwrite(posz, sizeof(float), nbodies, f);
#endif
    }
    endtime = clock();
