...
class PrecompMat{
 ...
 public:
  ...
  size_t CompactData(int level, Mat_Type type, Matrix<char>& comp_data, size_t offset=0){
    ...
    std::vector<Matrix<T> >& mat_=mat[(scale_invar?0:level+128)*Type_Count+type];
...
#pragma omp parallel for
	...
	if(M.Dim(0)>0 && M.Dim(1)>0){
	  ...
	  memcpy(comp_data[0]+offset_indx[j][0]+a*sizeof(T), &M[0][a], (b-a)*sizeof(T));
	}
	for(size_t l=l0;l<l1;l++){
	  Permutation<T>& Pr=Perm_R(l,type,j);
	  Permutation<T>& Pc=Perm_C(l,type,j);
...
#define MY_FREAD(a,b,c,d) {			\
  ...
  void LoadFile(const char* fname){
	...
	for(int j=0;j<n;j++){
	  ...
	  if(n1*n2>0){
	    M.Resize(n1,n2);
	    memcpy(&M[0][0], f_ptr, sizeof(T)*n1*n2); f_ptr+=sizeof(T)*n1*n2;
	  }
	}
      }
      perm_r.resize(256*Type_Count);
