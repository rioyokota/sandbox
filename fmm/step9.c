...
void P2M(double *M, double q) {
  *M += q;
}

void M2M(double *Mi, double Mj) {
  *Mi += Mj;
}

void M2L(double *L, double M, double r) {
  *L += M / r;
}

void L2L(double *Li, double Lj) {
  *Li += Lj;
}

void L2P(double *u, double L) {
  *u += L;
}

void P2P(double *u, double q, double r) {
  if (r!=0) *u += q / r;
}
...
int main() {
  ...
  for (l=level; l>2; l--) {
    ...
    for (i=0; i<Ncell[level]; i++) {
      j = cells[l][i];
      if (cells2[l][j]>=0)
	M2M(&M[cells2[l-1][j/4]+levelOffset[l-1]],M[i+levelOffset[l]]);
    }
  }
  // M2L
  int jc, jX[2];
	...
	if (abs(iX[0]/2-jX[0]/2) <= 1 && abs(iX[1]/2-jX[1]/2) <= 1) {
	  if (abs(iX[0]-jX[0]) > 1 || abs(iX[1]-jX[1]) > 1) {
	    ...
	    if (cells2[l][i]>=0&&cells2[l][j]>=0)
	      M2L(&L[cells2[l][i]+levelOffset[l]],M[cells2[l][j]+levelOffset[l]],r);
	  }
	}
      }
    }
  ...
  for (l=3; l<=level; l++) {
    for (i=0; i<Ncell[level]; i++) {
      j = cells[l][i];
      if (cells2[l][j]>=0)
	L2L(&L[i+levelOffset[l]],L[cells2[l-1][j/4]+levelOffset[l-1]]);
    }
  }
  // L2P
  for (i=0; i<Ncell[level]; i++) {
    for (j=offset[i]; j<offset[i+1]; j++) {
      L2P(&u[j],L[i+levelOffset[level]]);
    }
  }
  // P2P
