...
timeTuple2 superNode::compress()
{
  ...
  for ( unsigned int i = 0; i < outEdges.size(); i++ )
    ...
    times[1] += double(finish-start)/CLOCKS_PER_SEC;
    start = clock();
    Eigen::ColPivHouseholderQR<densMat> qrU( bigU );
    finish = clock();
    qrU.setThreshold( epsilon_ );
    ...
    *( inEdges.back() -> matrix ) = ( (densMat) (qrU.householderQ().setLength(qrU.nonzeroPivots())) ).leftCols(k);
    *( outEdges.back() -> matrix ) = inEdges.back() -> matrix -> transpose();

    densMat R = densMat::Zero( k, nCols );
    R = qrU.matrixR().topLeftCorner(k, nCols).template triangularView<Eigen::Upper>(); 
    R = R * qrU.colsPermutation().transpose();
    int firstRow = 0;
    for ( unsigned int i = 0; i < inEdges.size(); i++ )
      {
	// Check if the edge is well-separated
	if ( inEdges[i] -> isWellSeparated() )
	  {    
	    ...
	    *(redParent()->inEdges.back()->matrix) = R.block( 0, firstRow, k, nColsi );
	    inEdges[i] -> compress();
	    ...
	    redParent() -> outEdges.back() -> matrix  =  new densMat( nColsi, k );

	    *(redParent()->outEdges.back()->matrix) = ( R.block( 0, firstRow, k, nColsi ) ).transpose();
