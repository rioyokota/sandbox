...
void superNode::compress()
{
  ...
  for ( unsigned int i = 0; i < outEdges.size(); i++ )
    ...
    // Compress bigU = Unew * R and bigV = L * Vnew
    ...
    *( outEdges.back() -> matrix ) = ( svdU.matrixU().leftCols(k) ).transpose();
	...
	if ( inEdges[i] -> isWellSeparated() )
	  {    
	    ...
	    *(redParent()->outEdges.back()->matrix) = ( ( svdU.singularValues().head(k).asDiagonal() * ( svdU.matrixV().block( firstRow, 0, nColsi, k ) ).transpose() ) ).transpose();
...
int superNode::cutOff( RedSVD::RedSVD<densMat> &svd, int method, double epsilon, int apRank, double rankCapFactor )
{
  int return_value = 0;
