...
c        call prinf('ntarget=*',ntarget,1)
        print*,'FMM    =',t2-t1
c        call prin2('after fmm, time (sec)=*',t2-t1,1)
c        call prin2('after fmm, speed (points+targets/sec)=*',
c     $     (nsource+ntarget)/(t2-t1),1)
c       
...
c     $     (t2-t1)*dble(nsource)/dble(m),1)
c        call prin2('directly, estimated speed (points/sec)=*',
c     $     m/(t2-t1),1)
c       
        if (ifpot .eq. 1)  then
           call l3derror(pot,pot2,m,aerr,rerr)
           print*,'Err pot=',rerr
c           call prin2('relative L2 error in potential=*',rerr,1)
        endif
c
        if (iffld .eq. 1) then
           call l3derror(fld,fld2,3*m,aerr,rerr)
           print*,'Err acc=',rerr
c           call prin2('relative L2 error in field=*',rerr,1)
        endif
c       
c
        do i=1,ntarget
...
c           call l3derror(pottarg,pot2,m,aerr,rerr)
c           call prin2('relative L2 error in target potential=*',rerr,1)
c        endif
c
c        if (iffldtarg .eq. 1) then
c           call l3derror(fldtarg,fld2,3*m,aerr,rerr)
